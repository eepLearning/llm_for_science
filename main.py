...
def cmd_parse_pdf(args):
    """저장된 PDF 배치 파싱."""
    from pdf_parser import compare_pdf_backends, parse_pdfs_batch
...
def cmd_clean(args):
    """LaTeX → plain text 변환."""
    from latex_cleaner import clean_all_records
...
def cmd_stats(args):
    """품질 리포트."""
    from stats_report import generate_report, print_report
...
def cmd_eval(args):
    """Eval 점수화."""
    from eval_scorer import run_eval
...
def cmd_eval_history(args):
    """Eval 히스토리 출력."""
    from eval_scorer import print_eval_history
...
def main():
    parser = argparse.ArgumentParser(
        description="현대 화학 논문 크롤러 (arXiv + Semantic Scholar)",
